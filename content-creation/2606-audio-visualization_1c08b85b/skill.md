...
### `visualizeAudio()` TypeScript Signature
...
type MandatoryVisualizeAudioOptions = {
  ...
  frame: number;
  fps: number;
  numberOfSamples: number;  // Must be power of 2: 32, 64, 128, 256, 512, 1024
};

// Optional parameters:
type OptionalVisualizeAudioOptions = {
  optimizeFor: "accuracy" | "speed";  // ONLY these two values, default "accuracy"
  smoothing: boolean;                  // default true
  dataOffsetInSeconds: number;         // required when using useWindowedAudioData
};

// Returns: number[] — values 0-1, left = bass, right = highs
```

**Do NOT pass** `minFrequency`, `maxFrequency`, `type`, or other non-existent parameters.
The only valid `optimizeFor` values are `"accuracy"` and `"speed"` (NOT `"frequency"` or `"waveform"`).
