...
```markdown
[STATUS: Context Compaction] Context window above 85% capacity Initiating maintenance agents for context preservation...
```
...
1. **logging agent** - Update work logs in task file
   ...
   ```markdown
   [RUNNING: Logging Agent] Consolidating work logs and updating task documentation...
   ```
...
2. **context-refinement agent** - Check for discoveries/drift
   ...
   ```markdown
   [RUNNING: Context-Refinement Agent] Checking for new discoveries or context drift...
   ```
...
```markdown
[COMPLETE: Context Compaction] ✓ Work logs consolidated ✓ Context manifest [updated/current] ✓ Service documentation [updated/current]
