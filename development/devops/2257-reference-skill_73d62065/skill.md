...
### Naming Convention: `0-` Prefix for Root Skills

Root skills (`melt`, `heavy`, `repair`) use a `0-` directory prefix (e.g., `0-melt/`) so they sort to the top of the skills listing. This makes the primary entry-point commands easy to find among 28+ utility skills.

- The `0-` prefix applies only to **directory names**, not slash commands — `/melt`, `/heavy`, and `/repair` remain unchanged.
- The installer symlinks the entire `config/skills/` directory, so no install changes are needed.
...
**Location**: `~/.claude/skills/0-heavy/`
...
**Location**: `~/.claude/skills/0-repair/`
