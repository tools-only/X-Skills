...
| # | 任务描述 | 最佳工具 | 示例命令/调用 |
|---|---------|---------|--------------|
| 54 | CI/CD 流程设计 | cicd-automation plugin | 描述需求，生成工作流 |
| 55 | K8s manifest 生成 | kubernetes-operations plugin | 生成 Deployment/Service YAML |
| 56 | Terraform 配置 | cloud-infrastructure plugin | IaC 模板生成 |
| 57 | 数据库迁移 | database-migrations plugin | 生成迁移脚本 + 执行 |
| 58 | 云架构设计 | cloud-architect plugin | AWS/Azure/GCP 架构 |
| 59 | GitOps 工作流 | deployment-engineer plugin | ArgoCD/Flux 配置 |
| 60 | 容器安全扫描 | security-scanning plugin | 漏洞扫描 + 修复建议 |
