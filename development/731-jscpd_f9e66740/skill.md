...
| Target | Type | How |
|--------|------|-----|
| `plugins/holistic-linting/skills/holistic-linting/` | skill | Add jscpd as a pre-lint duplication detection step before running formatters and linters. The holistic-linting skill could invoke jscpd to identify copy-paste code blocks and suggest refactoring opportunities before format-lint-resolve workflows. |
| `plugins/holistic-linting/skills/holistic-linting/` | skill | Integrate jscpd with the holistic-linting-orchestrator agent to run duplication checks on modified files. When duplication threshold is exceeded, delegate to linting-root-cause-resolver to propose DRY refactoring. |
| `plugins/python3-development/skills/pre-commit/` | skill | Add jscpd as a pre-commit hook example in the pre-commit skill documentation. Show how to configure jscpd in `.pre-commit-config.yaml` for automatic duplication checking on commit, including threshold configuration and ignore patterns. |
| `plugins/python3-development/skills/python3-review/` | skill | Enhance the python3-review skill's "Code Structure" review dimension to include duplication detection using jscpd. Add a checklist item for code duplication percentage and suggest running jscpd when reviewing Python modules. |
| `.claude/agents/code-review.md` | agent | Extend the code-review agent's "LLM slop" detection to include duplication analysis using jscpd. When reviewing code changes, run jscpd on modified files to identify if LLM-generated code contains duplicated patterns that should be refactored. |

### New Skill Candidates

- **duplication-refactoring**: A skill that teaches Claude to detect code duplication using jscpd, analyze duplication reports (JSON/SARIF), identify refactoring candidates, and propose DRY refactoring strategies (extract function, create shared module, use inheritance/composition). The skill would integrate with holistic-linting workflows and pre-commit hooks.

### New MCP Server Candidates

- **jscpd-mcp**: An MCP server exposing jscpd capabilities as tools for Claude Code. Tools would include: `scan_for_duplicates` (run jscpd on paths with configurable thresholds), `get_duplication_report` (retrieve JSON/SARIF formatted results), `analyze_clone_groups` (provide refactoring suggestions for detected clones), and `configure_ignore_patterns` (manage jscpd ignore rules). This would enable real-time duplication detection during code review and refactoring workflows.

### Cross-References

- Related research: `research/developer-tools/repomix.md` — Both tools prepare codebases for AI consumption; repomix packs entire codebases for AI analysis while jscpd detects duplication. Combining them could enable AI-powered whole-codebase duplication analysis and refactoring recommendations.
- Related research: `research/developer-tools/niteni.md` — Both provide automated code review in CI/CD pipelines. Niteni focuses on AI-powered general code review while jscpd specializes in duplication detection. They complement each other as orthogonal quality checks in the same pipeline.
