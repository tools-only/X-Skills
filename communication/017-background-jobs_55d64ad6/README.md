# Background Jobs

| Property | Value |
|----------|-------|
| **Name** | Background Jobs |
| **Repository** | [Jeffallan/claude-skills](https://raw.githubusercontent.com/Jeffallan/claude-skills/main/skills/rails-expert/references/background-jobs.md) (⭐ 216) |
| **Original Path** | `skills/rails-expert/references/background-jobs.md` |
| **Category** | communication |
| **Subcategory** | email |
| **Tags** | communication |
| **Created** | 2025-12-15 |
| **Updated** | 2026-01-29 |
| **File Hash** | `55d64ad68649a6ad...` |

## Description

ruby
 Gemfile
gem 'sidekiq'
gem 'sidekiqcron'  Optional: scheduled jobs

**Tags:** `communication`

---

*This skill is maintained by [SkillFlow](https://github.com/tools-only/SkillFlow)*
*Source: [Jeffallan/claude-skills](https://raw.githubusercontent.com/Jeffallan/claude-skills/main/skills/rails-expert/references/background-jobs.md)*
