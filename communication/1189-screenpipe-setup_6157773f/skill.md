...
Enable ambient work intelligence - Dex learns what you've been working on without manual logging.

## Triggers
- `/screenpipe-setup`, `/screenpipe-enable`
- "enable screen capture", "setup screenpipe", "ambient intelligence"
...
Ensure `~/.screenpipe/config.json` has our privacy defaults (see config below).
...
```bash
echo "7" > ~/.screenpipe/retention_days   # 7 days
echo "90" > ~/.screenpipe/retention_days  # 90 days
echo "14" > ~/.screenpipe/retention_days  # 14 days
```
