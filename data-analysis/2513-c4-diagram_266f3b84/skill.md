...
| ------------- | -------- | --------------------------------------------------------- |
| `system-name` | Yes      | Name of the System note (e.g., `AlertHub`, `ERPSystem`, `CloudPlatform`) |
...
- **Strip wiki-link syntax** from target names (e.g., `[[System - AlertHub]]` → `AlertHub`)
...
**Example:** `/c4-diagram AlertHub both` → Generates C4 Context and Container diagrams from AlertHub's frontmatter
